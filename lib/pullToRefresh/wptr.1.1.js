var WebPullToRefresh = (function () {
	...
	var _panDown = function(e) {
		// if ( ! pan.enabled ) {
			// return;
		// }
		console.log("PAN DOWN ", e.distance);

		e.preventDefault();
		if (pan.regular){
			window.scrollBy(0, (e.distance / options.resistance) * -1);
			return;
		}
	...
	var _panUp = function(e) {
		// if ( ! pan.enabled || pan.distance === 0 ) {
			// return;
		// }
		console.log("PAN UP ", e.distance);

		e.preventDefault();
		if (pan.regular){
			window.scrollBy(0, (e.distance / options.resistance));
			return;
		}
	...
	var _setBodyClass = function() {
		if ( pan.distance > options.distanceToRefresh ) {
			...
			document.getElementById('ptrTextStart').style.display = 'none';
			document.getElementById('ptrTextEnd').style.display = 'inline-block';
		} else {
			bodyClass.remove( 'ptr-refresh' );
			document.getElementById('ptrTextStart').style.display = 'inline-block';
			document.getElementById('ptrTextEnd').style.display = 'none';
		}		
	};
